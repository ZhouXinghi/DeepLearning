...
#  apply(img, T.RandomHorizontalFlip(p=0.5))
#
#  apply(img, T.RandomVerticalFlip(p=0.5))
#
#  apply(img, T.RandomResizedCrop((200, 200), scale=(0.1, 1), ratio=(1.0 / 2.0, 2.0 / 1.0)))

#  apply(img, T.ColorJitter(brightness=0.5, contrast=0.5, saturation=0.5, hue=0.5))

shape_aug = T.RandomResizedCrop((200, 200), scale=(0.1, 1), ratio=(1.0 / 2.0, 2.0 / 1.0))
...
d2l.show_images([ all_images[i][0] for i in range(32) ], 4, 8, scale=0.8) # first element of the tuple is the image
print([all_images[i][1] for i in range(32)])
